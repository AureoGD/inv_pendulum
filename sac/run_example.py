...
def load_config(config_path="configs/continuous_sac_pendulum.yaml"
                ):  # Default to an example config
...
def get_env_action_spec(env_action_space):
    ...
    if isinstance(env_action_space, gym.spaces.Box):  # Continuous
        ...
        raise ValueError(
            f"Unsupported action space type: {type(env_action_space)}")
...
def main(args):
    ...
    eval_env = None
    eval_env_id = config.get('eval_env_id',
                             config['env_id'])  # Default to training env_id
    if eval_env_id:
        ...
        print(
            "No evaluation environment specified. Evaluation will use the training environment instance."
        )
        eval_env = env

    # Dynamically load the agent class
    agent_module_name = config.get(
        "agent_module",
        "sac_cas.sac_cas_agent")  # e.g., "sac_cas.sac_cas_agent"
    agent_class_name = config.get("agent_class",
                                  "SacCasAgent")  # e.g., "SacCasAgent"
    ...
    try:
        ...
        try:
            module = importlib.import_module(
                f".{agent_module_name}",
                package="dsac")  # Assuming top package is dsac
            AgentClass = getattr(module, agent_class_name)
    ...
    if config.get('use_encoder', False) and isinstance(env.observation_space,
                                                       gym.spaces.Dict):
    ...
    trainer = Trainer(
        ...
        training_total_timesteps=int(
            config.get('training_total_timesteps', 100000)),
        ...
        eval_frequency_timesteps=int(
            config.get('eval_frequency_timesteps', 10000)),
        ...
        encoder_mlp_hidden_dims=config.get(
            'encoder_mlp_hidden_dims',
            [64, 64]),  # Used if agent uses SimpleMLPEncoder
    ...
    print(
        f"Starting training for {config.get('training_total_timesteps', 100000)} timesteps..."
    )
    try:
        trainer.train()
