...
from cont_env.pendlum_disc_env import InvPendulumEnv
...
def main(args):
    ...
    if algo == "ppo":
        ...
        train_env = VecNormalize(base_train_env, norm_obs=True, norm_reward=False)

    else:
        raise ValueError(f"Unsupported algorithm: {algo}")

    base_eval_env = DummyVecEnv([lambda: RecordEpisodeStatistics(InvPendulumEnv(max_step=5000))])
    ...
    model.learn(total_timesteps=total_timesteps, callback=[checkpoint_callback, eval_callback], log_interval=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-envs", type=int, default=8, help="Number of parallel envs (PPO only)")
    parser.add_argument("--eval-freq", type=int, default=10_000)
    parser.add_argument("--checkpoint-freq", type=int, default=50_000, help="Frequency to save checkpoints (steps)")
